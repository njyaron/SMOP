...
#estimate time distribution to find long_time
import parseFile
import featureFilter
import featureGenerator
import featureComparer
...
filters = [filter]
feature_types = [featureGenerator.TRIGRAPH_TYPE]
people = featureComparer.generate_people(filters, feature_types, False)

ts = [f.mean+2*f.stdev for p in people for f in p.features.values() if f.count>1 and f.mean>0]


####
def make_bar_chart(names, values):
    import numpy as np
    ...
    ax.set_title('Occurances by keyword group')
