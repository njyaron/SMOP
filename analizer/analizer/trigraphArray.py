...
MINIMAL_COMMON_KEYS = 250 #maybe 180 is enough
BAD_COMPARISON = 1000

#Note: session is [(keys, Feature), (keys, Feature), ...
...
class Person:
    def __init__(self, name, path, filter):
        ...
        for event_session in event_sessions:
            session = featureGenerator.analyze(event_session, filter, featureGenerator.TRIGRAPH_TYPE)
    ...
    def get_common_trigraphs(self):
        "Returns amount of common trigraphs between each possible couple of sessions"
    ...
    def calc_variances(self):
        "Calulates and stores the similarity of each possible couple of sessions"
        self.vars = []
        for i in range(len(self.sessions)):
            for j in range(i+1,len(self.sessions)):
...
def load_all():
    filter = featureFilter.Filter(languages=trigraphFilter.ALL_LANGUAGES, 
